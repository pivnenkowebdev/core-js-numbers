...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  const lengthFirstVector = Math.sqrt(x1 ** 2) + Math.sqrt(y1 ** 2);
  const lengthSecondVector = Math.sqrt(x2 ** 2) + Math.sqrt(y2 ** 2);
  const result = Math.acos(
    scalarProduct / (lengthFirstVector * lengthSecondVector)
  );
  return result;
}
...
function toNumber(value, def) {
  const isNumberType = Number(value);
  if (typeof isNumberType === 'number' && !Number.isNaN(isNumberType)) {
    return isNumberType;
  }
  return def;
}
